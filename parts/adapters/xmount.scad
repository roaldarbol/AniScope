...
module x_mount(
    ...
    level = -magnet_size[1]/2;
        ...
        union(){
            difference(){
                union(){
                    corners(adapter_magnet_dims, makerbeam); // Central magnets
                    mirror([1,0,0]) corners(adapter_magnet_dims, makerbeam); // Central magnets
                    corners(dims, makerbeam); // Peripheral magnets
                    mirror([1,0,0]) corners(dims, makerbeam); // Peripheral magnets
                    cross(dims, makerbeam, magnet_size, wall_thick);
